...
#[cfg(target_arch = "wasm32")]
use web_sys::{window, EventTarget, Window as webWindow};
#[cfg(target_arch = "wasm32")]
...
impl Host for ExampleHostSystems {
    ...
    type Engine = js_wasm_runtime_layer::Engine;

    fn create_engine(_: &mut GeeseContextHandle<WingsHost<Self>>) -> Self::Engine {
        js_wasm_runtime_layer::Engine::default()
    }
}
...
async fn run() {
    ...
    .document()
    .expect("ERROR: Could not retrieve document")
    .get_element_by_id("canvas")
    .expect("ERROR: Could not retrieve anchor")
    .dyn_into::<web_sys::HtmlCanvasElement>()
    .expect("ERROR: Could not retrieve HtmlCanvasElement");
...
let builder = winit::window::WindowBuilder::new().with_canvas(Some(canvas));
    ...
    let selected_format = swapchain_capabilities.formats[0];
...
impl EguiRenderer {
    ...
    pub fn draw(
        ...
        let full_output = self.state.egui_ctx().run(raw_input, |_| {
            ...
            egui::Window::new("The Host says hello").show(&self.state.egui_ctx(), |ui| {
                ui.label("Hello from Host!");
            });
